...
def test_create_customer_success(db_session):
    ...
    customer = service.create(
        name="Maria Silva", email="maria@example.com", cpf=VALID_CPF, password="Password123"
    )
...
def test_create_customer_invalid_cpf(db_session):
    ...
    with pytest.raises(InvalidCpfError):
        service.create(
            name="Maria Silva", email="maria@example.com", cpf="12345678900", password="Password123"
        )
...
def test_create_customer_duplicate_cpf(db_session):
    ...
    with pytest.raises(DuplicateCpfError):
        service.create(
            name="Outro Nome", email="outro@example.com", cpf=VALID_CPF, password="Password123"
        )
